...
import { GererConventionsService } from '../convention-comite-basic-example/gerer-conventions.service';
...
@Component({
  ...
  templateUrl: './add-convention-dialog.component.html', // Corrected path
...
export class AddConventionDialogComponent { 
  ...
  tabs: any;
  constructor(private gservice : GererConventionsService, private service: ConventionComiteService,
    ...
    if (this.tabs == 0) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.service.refuserConvention(this.idCovention, this.convention).subscribe(data => {
            console.log(data);
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.service.refuserConventionPFE(this.idCovention, this.convention).subscribe(data => {
            console.log(data);
            this.service.getConventions().subscribe(data => {
              // this.conventions = data
              window.location.reload()
